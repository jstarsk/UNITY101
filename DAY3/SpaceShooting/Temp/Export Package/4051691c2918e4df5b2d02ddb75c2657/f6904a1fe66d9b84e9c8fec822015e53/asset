...
/////// Diffuse:
                ...
                float4 _noise = tex2D(_Map,TRANSFORM_TEX(_noiseUV, _Map));
                float4 time = _Time + _TimeEditor;
                float4 _cyclones = tex2D(_Map,TRANSFORM_TEX(i.uv0, _Map));
                float node_362 = (_Cyclones*_cyclones.r*0.6);
                ...
                float2 _finalUV = ((_cyclonesUV+(_Flow*time.r)*float2(0.1,0))+((1.0 - (_poles*_poles*_poles*_poles))*_noise.g)*float2(0.05,0));
                float4 _finalMap = tex2D(_Map,TRANSFORM_TEX(_finalUV, _Map));
                float3 _color = TriLerp( _finalMap.b , _Color1.rgb , _Color2.rgb , _Color3.rgb );
                float3 diffuseColor = _color;
                float3 diffuse = (directDiffuse + indirectDiffuse) * diffuseColor;
////// Emissive:
                float node_4168 = 0.0;
                fixed zero = 0.0;
                float opacity = saturate((zero + ( (max(0,dot(i.normalDir,viewDirection)) - zero) * (1.0 - zero) ) / ((1.0 - _Density) - zero)));
